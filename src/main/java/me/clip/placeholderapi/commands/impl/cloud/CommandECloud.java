...
public final class CommandECloud extends PlaceholderCommand {
  ...
  @Unmodifiable
  private static final List<PlaceholderCommand> COMMANDS =
      ImmutableList.of(
          new CommandECloudClear(),
  ...
  static {
    COMMANDS.forEach(
        command -> command.setPermission("placeholderapi.ecloud." + command.getLabel()));
  }

  @NotNull @Unmodifiable private final Map<String, PlaceholderCommand> commands;
  ...
  public void evaluate(
    ...
    if (!(target instanceof CommandECloudToggle)
        && !plugin.getPlaceholderAPIConfig().isCloudEnabled()) {
      Msg.msg(sender, "&cThe eCloud Manager is not enabled!");
      return;
    }
  ...
  public void complete(
    ...
    if (params.size() <= 1) {
      final Stream<String> targets =
          filterByPermission(sender, commands.values().stream())
              .map(PlaceholderCommand::getLabels)
