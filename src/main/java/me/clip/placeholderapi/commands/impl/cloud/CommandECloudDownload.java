...
public final class CommandECloudDownload extends PlaceholderCommand {
  ...
  public void evaluate(
    ...
    if (params.isEmpty()) {
      Msg.msg(sender, "&cYou must supply the name of an eCloud expansion.");
      return;
    }

    final CloudExpansion expansion =
        plugin.getExpansionCloud().getCloudExpansion(params.get(0)).orElse(null);
    if (expansion == null) {
      Msg.msg(sender, "&cCould not find expansion named: &f" + params.get(0));
      return;
    }
    ...
    plugin
        .getExpansionCloud()
        .downloadExpansion(expansion, version)
        .whenComplete(
            (file, exception) -> {
              if (exception != null) {
                Msg.msg(sender, "&cFailed to download expansion: &e" + exception.getMessage());
                return;
              }

              Msg.msg(sender, "&aSuccessfully downloaded expansion to file: &e" + file.getName());
  ...
  public void complete(
    ...
    if (params.size() <= 1) {
      final Stream<String> names =
          plugin.getExpansionCloud().getCloudExpansions().values().stream()
    ...
    final Optional<CloudExpansion> expansion =
        plugin.getExpansionCloud().getCloudExpansion(params.get(0));
