...
public final class CommandParse extends PlaceholderCommand {
  ...
  private void evaluateParseSingular(
      @NotNull final CommandSender sender,
      @NotNull @Unmodifiable final List<String> params,
    ...
    if (params.size() < 2) {
      Msg.msg(
          sender,
          "&cYou must supply a target, and a message: &b/papi "
              + (broadcast ? "bcparse" : "parse")
              + " &7{target} &a{message}");
      return;
    }
    ...
    final String message =
        PlaceholderAPI.setPlaceholders(player, String.join(" ", params.subList(1, params.size())));
  ...
  private void evaluateParseRelation(
      @NotNull final CommandSender sender, @NotNull @Unmodifiable final List<String> params) {
    if (params.size() < 3) {
      Msg.msg(
          sender,
  ...
  private void completeParseSingular(
      @NotNull final CommandSender sender,
      @NotNull @Unmodifiable final List<String> params,
      @NotNull final List<String> suggestions) {
    if (sender instanceof Player
        && (params.isEmpty()
            || (params.size() == 1 && params.get(0).toLowerCase().startsWith("m")))) {
      suggestions.add("me");
    }
  ...
  private void completeParseRelation(
