...
public final class CommandECloudExpansionList extends PlaceholderCommand {
  ...
  @Unmodifiable private static final Set<String> OPTIONS = ImmutableSet.of("all", "installed");
  ...
  public void evaluate(
    ...
    switch (params.get(0).toLowerCase()) {
      ...
      case "installed":
        ...
        break;
      default:
        builder.append("&bExpansions by &6").append(params.get(0));
        break;
    }

    builder.append(" &bPage&7: &a").append(page).append('\n');
  ...
  public void complete(
    ...
    if (params.size() <= 1) {
      suggestByParameter(
          Sets.union(OPTIONS, plugin.getExpansionCloud().getCloudAuthorNames()).stream(),
          suggestions,
    ...
    suggestByParameter(
        IntStream.rangeClosed(1, (int) Math.ceil((double) expansions.size() / PAGE_SIZE))
  ...
  @NotNull
  private static List<CloudExpansion> getPage(
      @NotNull final Map<Integer, CloudExpansion> expansions, final int page, final int pageSize) {
    ...
    return IntStream.range(head, tail)
        .mapToObj(expansions::get)
        .filter(Objects::nonNull)
        .collect(Collectors.toList());
  }

  @NotNull
  private static Map<Integer, CloudExpansion> getExpansions(
      @NotNull final String target, @NotNull final PlaceholderAPIPlugin plugin) {
