...
public final class CommandExpansionRegister extends PlaceholderCommand {
  ...
  public void evaluate(
    ...
    final PlaceholderExpansion expansion =
        plugin.getExpansionManager().registerExpansion(params.get(0));
    if (expansion == null) {
      Msg.msg(sender, "&cFailed to register expansion from &f" + params.get(0));
      return;
    }
  ...
  public void complete(
    ...
    final String[] fileNames =
        plugin.getExpansionManager().getFolder().list((dir, name) -> name.endsWith(".jar"));
    if (fileNames == null || fileNames.length == 0) {
      return;
    }

    suggestByParameter(
        Arrays.stream(fileNames), suggestions, params.isEmpty() ? null : params.get(0));
  }
}
