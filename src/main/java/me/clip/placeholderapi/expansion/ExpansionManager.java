...
public final class ExpansionManager {
  ...
  public boolean registerExpansion(@NotNull final PlaceholderExpansion expansion) {
    ...
    if (expansion instanceof VersionSpecific) {
      ...
      if (!nms.isCompatibleWith(PlaceholderAPIPlugin.getServerVersion())) {
        plugin
            .getLogger()
            .info(
                "Your server version is not compatible with expansion: "
    ...
    if (plugin.getPlaceholderAPIConfig().isCloudEnabled()) {
      final CloudExpansion cloudExpansion =
          plugin.getExpansionCloud().getCloudExpansion(expansion.getIdentifier()).orElse(null);
  ...
  public PlaceholderExpansion registerExpansion(@NotNull final String fileName) {
    final List<Class<? extends PlaceholderExpansion>> subs =
        FileUtil.getClasses(folder, PlaceholderExpansion.class, fileName);
    if (subs.isEmpty()) {
      return null;
    }
  ...
  public void registerAllExpansions() {
    final List<@NotNull Class<? extends PlaceholderExpansion>> subs =
        FileUtil.getClasses(folder, PlaceholderExpansion.class);
    if (subs.isEmpty()) {
      return;
    }
    ...
    for (final Class<? extends PlaceholderExpansion> clazz : subs) {
      ...
      try {
        ...
        plugin
            .getLogger()
            .log(
                Level.WARNING, "Couldn't register " + expansion.getIdentifier() + " expansion", ex);
      }
    }
  }

  @Nullable
  private PlaceholderExpansion createInstance(
    ...
    try {
      ...
      plugin
          ...
          .log(
              Level.SEVERE,
              "Failed to load placeholder expansion from class: " + clazz.getName(),
              ex);
    }
